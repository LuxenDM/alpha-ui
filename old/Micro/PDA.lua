...
local function createChatBox(vertsize)
	print("Creating new PDA chat box")
	
	local outbox = iup.multiline {
		readonly = "YES",
		expand = "HORIZONTAL",
	...
	if vertsize == 1 then
		outbox.size = HUDSize(0.5, 0.1)
		outbox.expand = "YES"
	end
	
	
	
	Micro.newChatBox(function(chat) outbox.value = chat end)
	
	local inbox = iup.text {
		expand = "HORIZONTAL",
		value = "",
		font = Font.H5,
		wanttab = "YES",
...
local function createMerchBox()
	...
	local function createShipStatus()
		...
		local function updateUsrData()
			local details = {GetCharacterInfo(Micro.lastCharLogin or 1)}
...
local diag = iup.dialog {
	...
	iup.vbox {
		iup.label {
			title = Micro.main .. " " .. Micro.version,
		},
		iup.fill {
			size = "%1",
		},
